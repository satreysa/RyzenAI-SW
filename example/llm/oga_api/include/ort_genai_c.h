...
/**
/* 
 * \brief Creates a model from the given configuration directory and device type. Uses in-memory ONNX model that is already loaded.
 * \param[in] config_path The path to the model configuration directory. The path is expected to be encoded in UTF-8.
 * \param[in] model_data Pointer to model data
 * \param[in] model_data_length Size of the model data in bytes
 * \param[in] external_data Pointer to model external data
 * \param[in] external_data_length Size of the model external data in bytes
 * \param[in] header_data Pointer to proto header
 * \param[in] header_data_length Size of the proto header in bytes
 * \param[out] out The created model.
 * \return OgaResult containing the error message if the model creation failed.
 */
OGA_EXPORT OgaResult* OGA_API_CALL OgaCreateModelFromMemory(const char* config_path, const void* model_data, size_t model_data_length, const void* external_data, size_t external_data_length, const void* header_data, size_t header_data_length, OgaModel** out);
