...
struct OgaModel : OgaAbstract {
  //API for loading model from disk
  ...
  //API for using in-memory model
  //*_length args is size in bytes
  //the 3 sets of args are for *.onnx/*.onnx.data/*.pb.bin files
  static std::unique_ptr<OgaModel> Create(const char* config_path, const void* model_data, size_t model_data_length, const void* external_data, size_t external_data_length, const void* header_data, size_t header_data_length) {
    OgaModel* p;
    OgaCheckResult(OgaCreateModelFromMemory(config_path, model_data, model_data_length, external_data, external_data_length, header_data, header_data_length, &p));
    return std::unique_ptr<OgaModel>(p);
  }
